def save_data_rhu(request):
    ...
    request_json['rh'] = float(request_json['rh'])
    ...
    db.collection('rhu').document(sdatetime).set(request_json)

    doc_ref = db.collection('training_rhu').document('last_train')
    doc = doc_ref.get()
    if doc.exists:
    ...
    if (request_json['datetime'] - datetime.strptime(last_train, '%Y-%m-%d %H:%M:%S')).days >= 7:
        import requests
        r = requests.post('https://europe-west8-pcloud2026-495807.cloudfunctions.net/train_model_rhu')
